...
- **`Session::HandleRead()`**:
  - **全双工分离**: 它是“读”的终点。收到数据后，它执行两个操作：
    1.  调用 `Send()` 将数据放入发送队列（触发写操作）。
    2.  立即再次调用 `async_read`，保持对客户端数据的持续监听。
  - **生命周期传递**: 它接收 `_self_shared` 参数，确保在处理数据期间 Session 不死。
...
sequenceDiagram
    ...
    loop Echo Loop
        ...
        par Process
            Session->>Session: Send() -> Queue -> async_write
            Session->>Session: async_read (Loop)
        end
        Session->>Client: ACK / Echo Data
        Session->>Session: HandleWrite (Pop Queue)
    end
```
